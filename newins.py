...
SIMILARITY_THRESHOLD = 1.1  # Чем меньше, тем строже
...
def rename_image():
    ...
    if not old_name or not new_name:
        return jsonify({"error": "Не передано oldName или newName"}), 400

    # Найти файл с поддерживаемым расширением
    old_file_path = None
    file_ext = None
    for ext in ALLOWED_EXTENSIONS:
        candidate = os.path.join(KNOWN_FACES_DIR, f"{old_name}.{ext}")
    ...
    if not old_file_path:
        return jsonify({"error": f"Файл '{old_name}' не найден"}), 404

    new_file_path = os.path.join(KNOWN_FACES_DIR, f"{secure_filename(new_name)}.{file_ext}")
    ...
    try:
        ...
        global known_face_encodings, known_face_names
        known_face_encodings, known_face_names = load_known_faces()
        return jsonify({"status": f"Файл переименован в '{new_name}.{file_ext}'"}), 200
    except Exception as e:
        return jsonify({"error": f"Ошибка при переименовании: {str(e)}"}), 500
...
def rebuild_embeddings():
    try:
        ...
        known_face_encodings, known_face_names = load_known_faces()
        return jsonify({
            "status": "Эмбеддинги успешно пересозданы",
            "count": len(known_face_names),
            "names": known_face_names
        }), 200
    except Exception as e:
        return jsonify({"error": f"Ошибка при пересоздании эмбеддингов: {str(e)}"}), 500
